...
const getResolvedMaxMark = (exam: any, subjectId: string, shortCode: string, defaultMax = 50) => {
  ...
  if (c.includes('P01') || c === 'AT' || c.includes(' AT') || c.includes('(AT)')) {
    val = getVal('P01');
    if (val === undefined) val = getVal('Lan I');
    if (val !== undefined) return val;
  }
  if (c.includes('P02') || c === 'BT' || c.includes(' BT') || c.includes('(BT)')) {
    val = getVal('P02');
    if (val === undefined) val = getVal('Lan II');
    if (val !== undefined) return val;
  }
...
async function findMarksGroupedByStudent(examId: string, studentIds: string[]) {
  ...
  markentries.forEach((entry: any) => {
    ...
    studentMarksMap[studentId][subjectCode] = {
      grade: entry.grade || '',
      mark: entry.mark !== undefined ? entry.mark : (entry.grade ? '' : ''),
  ...
  marksList.forEach(m => {
    ...
    for (const code of allowedSubjectCodes) {
      ...
      if (!isNaN(numericMark) && String(valToUse).trim() !== '') {
        ...
        else if (pct >= 35) filteredGrades.push('D+');
...
app.get("/api/management/grades", authenticateToken, requireRole('WEBMASTER', 'DEO', 'DIET', 'SCHOOL', 'TEACHER'), async (req, res) => {
  try {
    ...
    const data = {
      std9_10: transform(gradeDoc.std9_10, ['25', '30', '35', '40', '80']),
    ...
    for (const [sid, pm] of prevMarksByStudent) {
      ...
      for (const code of allowedSubjectCodes) {
        ...
        if (!isNaN(numericMark) && String(valToUse).trim() !== '') {
          ...
          else if (pct >= 35) filteredGrades2.push('D+');
...
// ─── Dashboard: Subject-wise Counts (First Language P01-P04 + Core P05-P10) ───
app.get("/api/dashboard/subject-counts", async (req: any, res) => {
  try {
    ...
    // Resolve school IDs in scope (same logic as /api/dashboard/stats)
    ...
    // Single aggregation pipeline with $facet for both groupings in one DB round-trip
    const [facetResult] = await Student.aggregate([
      { $match: matchFilter },
      {
        $facet: {
          // Group by firstLangPaper1 for P01 counts
          byFirstLangPaper1: [
            { $group: { _id: "$firstLangPaper1", count: { $sum: 1 } } },
            { $match: { _id: { $ne: '' } } },
            { $sort: { count: -1 } }
          ],
          // Group by firstLangPaper2 for P02 counts
          byFirstLangPaper2: [
            { $group: { _id: "$firstLangPaper2", count: { $sum: 1 } } },
            { $match: { _id: { $ne: '' } } },
            { $sort: { count: -1 } }
          ],
          // Group by secondLang for P03 counts
          bySecondLang: [
            { $group: { _id: "$secondLang", count: { $sum: 1 } } },
            { $match: { _id: { $ne: '' } } },
            { $sort: { count: -1 } }
          ],
          // Group by thirdLang for P04 counts
          byThirdLang: [
            { $group: { _id: "$thirdLang", count: { $sum: 1 } } },
            { $match: { _id: { $ne: '' } } },
            { $sort: { count: -1 } }
          ],
          // Total students with gender for overall count
          totalByGender: [
            { $group: { _id: "$gender", count: { $sum: 1 } } }
          ],
          // Group by medium + gender for medium-wise counts
          byMediumGender: [
            { $match: { medium: { $ne: '' } } },
            { $group: { _id: { medium: "$medium", gender: "$gender" }, count: { $sum: 1 } } },
            { $sort: { "_id.medium": 1, "_id.gender": 1 } }
          ],
          totalCount: [
            { $count: "total" }
          ]
    ...
    // Build first languages (P01-P04)
    const firstLanguages = [
      { code: 'P01', label: 'First Language Paper I', data: facetResult.byFirstLangPaper1 || [] },
      { code: 'P02', label: 'First Language Paper II', data: facetResult.byFirstLangPaper2 || [] },
      { code: 'P03', label: 'Second Language', data: facetResult.bySecondLang || [] },
      { code: 'P04', label: 'Third Language', data: facetResult.byThirdLang || [] },
    ];

    const genderTotals: Record<string, number> = {};
    (facetResult.totalByGender || []).forEach((g: any) => {
      genderTotals[g._id || 'Unknown'] = g.count;
    });

    const totalStudents = facetResult.totalCount?.[0]?.total || 0;

    // Build medium-wise counts — normalize all medium values to canonical shortName
    const mediumDocs = await Medium.find({ active: true }).lean();
    const mediumNorm: Record<string, string> = {}; // input -> canonical shortName
    mediumDocs.forEach((m: any) => {
      ...
      if (sn) {
        mediumNorm[sn.toLowerCase()] = sn;
        mediumNorm[sn.toUpperCase()] = sn;
      }
      if (code) mediumNorm[code] = sn || code;
      if (id) mediumNorm[id] = sn || id;
      if (name) mediumNorm[name] = sn || name;
    });

    // Hardcoded fallback normalization
    ...
    const normalizeMedium = (raw: string): string => {
      const trimmed = (raw || '').trim();
      if (!trimmed) return '';
      const lower = trimmed.toLowerCase();
      if (mediumNorm[lower]) return mediumNorm[lower];
      if (defaultNorm[lower]) return defaultNorm[lower];
      // Title-case fallback
      return trimmed.charAt(0).toUpperCase() + trimmed.slice(1).toLowerCase();
    };

    const mediumRaw: Record<string, { male: number; female: number; total: number }> = {};
    (facetResult.byMediumGender || []).forEach((item: any) => {
      const rawMedium = item._id?.medium || 'Unknown';
      const normalizedMedium = normalizeMedium(rawMedium);
      const gender = item._id?.gender || 'Unknown';
      const count = item.count || 0;
      if (!mediumRaw[normalizedMedium]) mediumRaw[normalizedMedium] = { male: 0, female: 0, total: 0 };
      if (gender === 'Male' || gender === 'Boy') mediumRaw[normalizedMedium].male += count;
      else if (gender === 'Female' || gender === 'Girl') mediumRaw[normalizedMedium].female += count;
    ...
    const mediumCounts = Object.entries(mediumRaw)
      ...
      .map(([name, counts]) => ({
        code: shortNameToCode[name.toUpperCase()] || name,
        name,
        ...counts,
      }))
    ...
    const response = {
      ...
      mediumCounts,
      totalStudents,
      maleCount: genderTotals['Male'] || genderTotals['Boy'] || 0,
      femaleCount: genderTotals['Female'] || genderTotals['Girl'] || 0,
    };
...
app.get("/api/dashboard/school-analysis", async (req: any, res) => {
  try {
    ...
    const studentFilter: any = { schoolId, className: examClass };
    ...
    if (studentIds.length === 0) {
      const anyStudents = await Student.find({ schoolId }).lean();
...
async function computeSchoolAnalysis(schoolId: string, examId: string, exam: any, students: any[], rawMarks: any[], res: any, analyticsCache: any, cacheKey: string) {
  ...
  const getResolvedMaxMark = (subjectId: string, shortCode: string): number => {
    if (exam?.maxMarks?.[subjectId]) return exam.maxMarks[subjectId] as number;
    if (shortCode === 'P01' || shortCode === 'P02') return 50;
    if (shortCode === 'P10') return 25;
    return 80;
  };
  ...
  try {
    ...
    if (exam) {
      ...
      if (exam.confirmations) {
        exam.confirmations.delete(schoolId);
      }
      if (exam.confirmedSubjects) {
        exam.confirmedSubjects.delete(schoolId);
      }
      await exam.save();
    ...
    for (const schoolId of schoolIds) {
      if (action === 'confirm') {
        if (!exam.confirmedSchools.includes(schoolId)) {
          ...
          if (!exam.confirmations) exam.confirmations = new Map();
          exam.confirmations.set(schoolId, `${new Date().toISOString()}|${req.user.username || req.user.id}`);
          confirmedCount++;
        }
      } else {
        exam.confirmedSchools = (exam.confirmedSchools || []).filter((sid: string) => sid !== schoolId);
        if (exam.confirmations) exam.confirmations.delete(schoolId);
        if (exam.confirmedSubjects) exam.confirmedSubjects.delete(schoolId);
        await Mark.updateMany(
          { examId, schoolId },
          { $set: { finalLocked: false, locked: false } }
        );
  ...
  try {
    ...
    if (finalConfirm) {
      ...
      if (exam) {
        ...
        if (!exam.confirmations) exam.confirmations = new Map();
        exam.confirmations.set(schoolId, `${new Date().toISOString()}|${req.user.username || 'System'}`);
        await exam.save();
      }
...
app.get("/api/marks/batch", authenticateToken, async (req, res) => {
  try {
    ...
    const query: any = { schoolId, className: className || '10' };
    if (division) {
    ...
    const isSchoolConfirmed = exam?.confirmedSchools?.includes(String(schoolId));
    ...
    const schoolConfig = await SchoolExamConfig.findOne({ schoolId: String(schoolId), examId: String(examId) });
...
// ─── MARK ENTRY STATUS (PER-SUBJECT PROGRESS) ───────────────────────────────
...
app.get("/api/marks/entry-status", authenticateToken, async (req: any, res) => {
  try {
    ...
    for (const sub of sortedSubjects) {
      ...
      const marksEnteredIncludingAbsent = await Mark.countDocuments({
        schoolId: effectiveSchoolId,
        examId,
        subjectId,
      });

      const effectiveEntered = Math.max(marksEntered, marksEnteredIncludingAbsent);
      const percentage = totalStudents > 0 ? Math.round((effectiveEntered / totalStudents) * 100) : 0;
      ...
      if (effectiveEntered === 0) {
        status = 'Not Yet Started';
      } else if (effectiveEntered >= totalStudents && isSubjectConfirmed) {
        status = 'Completed';
      } else {
        status = 'Pending';
      }
      ...
      // teachingSubjects stores subject names like "TAMIL TM", "MATHEMATICS", etc.
      // subjectDoc.name is the full name, subjectDoc.shortName is "P01", etc.
      ...
      const assignedTeachers = teachers
        .filter((t: any) => {
          ...
          return tSubs.some((ts: string) => {
            ...
            // Match by original name without normalization
    ...
    const overallPercentage = totalApplicableAcrossAll > 0
      ? Math.round((totalMarksEnteredAcrossAll / totalApplicableAcrossAll) * 100)
      : 0;
    ...
    res.json({
      ...
      overall: {
        totalStudents,
        totalMarksEntered: totalMarksEnteredAcrossAll,
...
app.post("/api/marks/entry2", authenticateToken, async (req: any, res) => {
  try {
    ...
    if (reset) {
      ...
      if (exam.confirmedSubjects) {
        const currentConfirmed = exam.confirmedSubjects.get(effectiveSchoolId) || [];
        const updated = currentConfirmed.filter((sid: string) => sid !== subjectId);
        exam.confirmedSubjects.set(effectiveSchoolId, updated);
    ...
    if (confirm) {
      ...
      if (isCoreSubject) {
        ...
        // Only count marks for the same exam, subject, and school.
        // We must count only marks for students actually in this standard, but since exam is bound to a standard,
        // all marks for this examId/subjectId/schoolId belong to these students.
        ...
        if (enteredMarks < totalStudents) {
          return res.status(400).json({
            message: `Validation Failed: Marks must be entered for all divisions. Only ${enteredMarks} out of ${totalStudents} students have marks for this subject.`
          });
        }
      }

      if (!exam.confirmedSubjects) exam.confirmedSubjects = new Map();
      const currentConfirmed = exam.confirmedSubjects.get(effectiveSchoolId) || [];
      if (!currentConfirmed.includes(subjectId)) {
        currentConfirmed.push(subjectId);
        exam.confirmedSubjects.set(effectiveSchoolId, currentConfirmed);
        exam.markModified('confirmedSubjects');
        await exam.save();
      }
      ...
      if (schoolConfig) {
        ...
        if (subIndex > -1) {
          ...
          // 4-stage workflow: set subject workflow status
...
app.post("/api/marks/school-confirm", authenticateToken, async (req: any, res) => {
  try {
    ...
    if (!exam.confirmations) exam.confirmations = new Map();
    exam.confirmations.set(effectiveSchoolId, `${new Date().toISOString()}|${req.user.username || req.user.id}`);
    await exam.save();
